...
Smartphone installiert und ausgeführt werden. Betrachtet man die gesammelten
Daten des Plug-Ins wird man feststellen, dass die zuvor definierte Datenstruktur
mit den entsprechenden Positionsdaten gefüllt wurden.
...
\subsubsection{Repräsentation der Daten}
...
Listing \ref{recordaudio} ergänzt werden. Bei Verwendung dieser {\em Permission}
sei darauf hingewiesen, dass der Benutzer eine Warnung bei der Aktivierung
erhält, da durch die Aufnahme von Ton personenbezogene Daten erhoben werden
können. Der {\em Mobile Data Collector} ist aber nicht in der Lage zu
...
\begin{itemize}
  \item Das Erstellen von Plug-Ins die praktisch relevante Daten, wie die
  Position und Umgebungsinformationen z.B. die Lautstärke erfassen, sind mit
  dem {\em Mobile Data Collection Frameworks} möglich zu erstellen.
  ...
  bzw. erweitern.
  \item Die Daten lassen anonym an externe Server übertragen und persistieren.
  \item Die Visualisierung hat gezeigt, dass die Daten auch Verwertbar sind.
\end{itemize}

Anforderungen an Transparenz und Anonymität werden eingehalten
