...
entwickeln lassen. Dies wird anhand zweier Beispiele gezeigt. Bei diesen
Beispielen handelt es sich zum einen um ein Plug-In zur Positionsverfolgung
eines Benutzers sowie um eine Erweiterung des Plug-Ins, dass zusätzlich die
Umgebungslautstärke zu jeder Position erfasst. Hierbei wird neben den Plug-Ins
für das Smartphone auch eine entsprechende Serverkomponente entwickelt. Die
Serverkomponente beinhaltet dabei, die Persistierung der Daten sowie deren
Visualisierung in Form einer Web-Anwendung.
...
trägt den Arbeitstitel {\em Location Tracker}. Es wird dabei auf die
wichtigsten Punkte bei der Implementierung des Plug-Ins sowie der Serverkomponente
eingegangen.
...
und diese Daten zu speichern. Das Plug-In befindet sich im {\em
...
der zurzeit beste verfügbare {\em Provider} zurückgegeben. In Zeile 2 wird nun
die letzte bekannte Position unter Zuhilfenahme des gegebenen {\em Providers}
...
Dann wird eine neue {\em Node} erstellt, an der alle Attribute des {\em
Location}-Objekts gespeichert werden. Dieses wird im letzten Schritt an den {\em
Workspace} angehangen bevor dieser gespeichert wird.
...
Diese Konfiguration unterscheidet sich zum ``Hallo Welt''-Plug-In dadurch, dass
der {\em LocationManager} angefordert wird und eine Beschreibung angegeben ist.
Der {\em LocationManager} wird zur Ermittlung der aktuellen Position benötigt.
...
Collection Frameworks} in ein domänenspezifisches Datenformat stattfinden. Eine
Überführung des allgemeinen Datenformats in ein domänenspezifisches, sowie die
anschließende Speicherung der Daten, kann dabei wie in Listing
...
der \acf{orm} {\em Hibernate} verwendet. Dieser wird vom {\em ParticipantRepsitory}
...
Daraufhin startet der \acs{gwt} eigene Webserver. Dies beinhaltet auch das
...
Ausgabeformat und Audio-Encoder sind zwar notwendig aber in diesem Fall egal, da
wie in Zeile 5 angegeben alle Daten im {\em null-Device} gespeichert werden.
...
bestimmt. Diese kann über die {\em getAmplitude}-Methode abgefragt werden. Damit
diese Methode einen Wert zurückliefert, muss für eine bestimmte Zeitspanne
eine Tonaufnahme erfolgen. In diesem Fall werden für eine Sekunde lang alle
Umgebungsgeräusche aufgenommen und die maximale Amplitude bestimmt. Durch die
{\em stop}- und {\em release}-Methode wird die Aufnahme angehalten und das
Mikrofon wieder freigegeben. Diese Art zur Bestimmung der Lautstärke wurde dem
...
unterschiedlich sein kann. Bei einer Auswertung ist deswegen eine Normalisierung
...
Das Plug-In ist nun fertig angepasst und kann zur Ermittlung der Lautstärke
verwendet werden.
...
\begin{itemize}
  \item Das Erstellen von Plug-Ins für reale Anwendungsszenarien, wie die
  Erfassung von Position und Umgebungsinformationen z.~B. die Lautstärke, sind
