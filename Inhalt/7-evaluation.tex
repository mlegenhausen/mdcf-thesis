...
durch die es möglich wird die aktuelle Position eines Benutzers zu ermitteln und
...
die letzte bekannte Position unter Zuhilfenahme des gegeben {\em Provider}s
...
{\em Node} erstellt, an der alle Attribute des {\em Location}-Objekts
...
Aufgabe zu erfüllen. In diesem Fall benötigt das Plug-In den {\em
location}-Service. Dieser wird daraufhin bei jeder Ausführung dem Plug-In zur
Verfügung gestellt. Folgende Services können zurzeit angefordert werden:
...
die anschließende Speicherung der Daten, kann dabei wie in Listing
...
Durch die {\em prepare} und {\em start}-Methode wird das Mikrofon reserviert und
die Aufnahme gestartet. Die Lautstärke wird nun anhand der maximalen Amplitude
bestimmt. Diese kann über die {\em getAmplitude}-Methode abgefragt werden. Damit
diese Methode einen Wert zurückliefert, muss für einen bestimmten Zeitintervall
...
Mikrofon wieder freigegeben. Diese Art der Lautstärkenbestimmung wurde dem
Beispielprogramm {\em NoiseAlert} entnommen\cite{noisealert}. Bei dieser Art der
Lautstärkenbestimmung ist zu erwähnen, dass die maximale Amplitude zwischen
verschiedenen Geräten unterschiedlich sein kann. Bei einer Auswertung ist
deswegen eine Anpassung der Daten erforderlich.
...
Bereich des Loggings müssen, beim Zugriff auf Ton und Video, bei der
derzeitigen Implementierung des Plug-Ins-System, Abstriche gemacht werden.