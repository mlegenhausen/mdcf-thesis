...
Ein Erweiterungssystem oder wie hier benannt {\em Plug-In System} kümmert sich
...
Zur Realisierung eines Plug-In-Systems auf Android gibt er zwei Ansätze. Der
...
zwei Ansätze sollen im Folgenden genauer betrachtet werden.

\subsection{Evaluation der \acs{osgi}-Plattform für Android}
Die Verwendung von \acs{osgi} als Modularisierungs- bzw. Plug-In-System würde
...
resultierende nötige Anpassung von \acs{osgi} an Android. Der Aufwand war
...
eines eigenen Plug-In-Systems war somit die einzige Lösung.
...
\begin{itemize}
  ...
  heruntergeladenen \acf{jar}. Das wäre der Ansatz von \acs{osgi}.
...
vorhanden. Auch ist Letzteres der Ansatz, der vom Android-System am besten unterstützt wird.

Plug-Ins können somit als {\em Service} angesehen werden. Nach der Installation
eines Plug-Ins sollen sich neue Plug-Ins beim Plug-In-System registrieren. Dazu
soll das Plug-In-System, nachdem es über die Installation eines neuen Plug-Ins
...
\begin{itemize}
  ...
  \item Die {\bf \acs{url}}, an die die gesammelten Daten übertragen werden
  sollen.
  ...
  \item Der {\bf Intervall}, in dem die Daten regelmäßig übertragen werden
  sollen.
  ...
  Über\-tragung gelöscht} werden sollen.
\end{itemize}
...
Ein Plug-In soll bestimmte Zustände einnehmen können. Nach der Installation soll
...
Zustand aus soll das Plug-In aktiviert werden können. Das heißt, das Plug-In
wird in der bei der Registrierung angegebenen Periode ausgeführt. Soll ein
...
haben, soll es in den Transferzustand übergeben. Die Zeitspanne soll bei der
Plug-In-Registrierung angegeben werden. In diesem Zustand sollen die Daten für
die Übertragung aufbereitet und lokal gespeichert werden. Wann die Daten
endgültig übertragen werden, soll der Benutzer selbst entscheiden. Siehe hierzu
den Abschnitt \ref{periodicallytransfer}. Sind die Daten aufbereitet und
gespeichert worden, geht das Plug-In wieder in den aktivierten Zustand über. Der
...
Möchte ein Benutzer ein Plug-In wieder entfernen, kann er das über den
Anwendungsmanager von Android machen. Darüber hinaus soll der Benutzer auch direkt die
Möglichkeit besitzen, das Plug-In aus der Übersicht zu entfernen. 

Bei der Deinstallation eines Plug-Ins erhält das Plug-In-System ebenfalls einen
{\em Broadcast} darüber, dass eine Anwendung entfernt wurde. Das erlaubt dem
Plug-In-System, die gespeicherten Daten vom Plug-In zu löschen und die
...
\begin{itemize}
  ...
  dürfen nicht im Plug-In selbst gespeichert, sondern müssen zentral in
  der Anwendung abgelegt werden.
  \item Bevor die Daten übertragen werden, muss der Benutzer darauf hingewiesen
  werden, damit eine Kontrolle der Daten vor der Übertragung stattfinden kann.
...
Die Voraussetzungen werden im Folgenden genauer erläutert.
...
System Services} zur Verfügung gestellt werden, sondern stattdessen überwachte
...
gefährliche Methoden gar nicht erst anbieten. Folgende {\em Android System
Services} müssen durch sichere Implementierung ersetzt werden:
...
Services} übergeben werden, sondern nur die, die in den Plug-In-Informationen
...
Service} zu erlangen, kann über die angeforderten Rechte des Plug-Ins entnommen
werden. Es muss also vor der Aktivierung eine Kontrolle stattfinden.

Das entwickelte Konzept erweitert die von Android zur Verfügung gestellte
Sandbox, um weitere Überwachungsmechanismen, damit der Schaden, der durch ein
Plug-In entstehen kann verringert wird. Der hierbei größte anzunehmende Schaden
...
Um die von einem Plug-In gesammelten Daten einheitlich präsentieren zu können,
muss ein entsprechendes Datenformat gewählt werden, an das sich alle Plug-Ins halten
...
Datenstruktur zusammensetzen lassen. Im Rahmen dieser Masterarbeit soll eine eingeschränkte
...
mit dem Namen {\em Item}. Die Klasse enthält eine Obermenge von Attributen die
...
aufgeführt. Wählt man  einen Knoten aus, so soll man in die nächste
Hierarchiestufe gelangen. Über die Zurücktaste soll man wieder eine
Hierarchiestufe zurück gelangen. Als Darstellungswert innerhalb der Liste soll
bei einem Knoten das Modifizierungsdatum und bei einer Eigenschaft eine
Kombination aus dem Namen und der String-Repräsentation des Wertes verwendet
werden.
...
Daten beim Plug-In selbst nachfragen zu müssen. So muss das Plug-In selbst keine
Daten speichern, die Persistierung wird von der Anwendung
übernommen. Die Speicherung der Daten soll per \acs{db4o} erfolgen. \acs{db4o}
...
Bei der Ausführung eines Plug-Ins soll dieses vollen Zugriff auf die
zuvor gespeicherten Daten besitzen, sodass die Möglichkeit besteht zuvor
...
Im Gegensatz zu anderen Anwendungen, bei denen die Daten sofort ohne Rück\-frage
des Benutzers übertragen werden, sollen sie in dieser Anwendung nur in regelmäßigen
Abständen und nur nach Erlaubnis des Benutzers übertragen werden. Die Abstände
sollen durch das Plug-In selbst definiert werden. Wird als Übertragungsintervall
z.~B. ein Tag angegeben, so soll der Benutzer, wenn das Plug-In einen Tag lang
aktiv gewesen ist, in den zuvor beschriebenen Transferzustand übergehen. Sollte
...
vom Plug-In gesammelten Daten gesichert und für die Übertragung vorbereitet
werden. Das Plug-In kann nun wieder aktiviert werden und weiter Daten sammeln.
Der Benutzer soll vor der Übertragung alle angefallenden Daten des Plug-Ins
einsehen können, das heißt, alle Zugriffe auf die {\em Android System Services}
...
daraufhin an die, in den Plug-In-Information enthaltenen \acs{url} übertragen
werden. Sollte der Benutzer nicht mit der Übertragung der Daten einverstanden sein, so
...
stattfinden kann, kann es passieren, dass sich mehrere Übertragungen ansammeln.
Alle Übertragungen sollen übersichtlich aufgelistet werden.

Bei der Übertragung müssen neben den gesammelten Daten, auch
...
Dazu soll auf dem Server ein \acs{rest}ful Webservice zur Verfügung gestellt
werden, um die von der Anwendung in \acs{json} codierten Daten zu empfangen.
\acs{json} erlaubt die Codierung von Daten in einem für Menschen lesbaren
Format, besitzt im Vergleich zu \acs{xml} einen geringen Overhead und ist ein
mittlerweile anerkannter Standard zur Übertragung von Daten übers
Internet~\cite{json}.
...
domänen\-spezi\-fisches Datenformat stattfinden und muss vom Plug-In-Entwickler
selbst vorgenommen werden. Das ist aus den folgenden Gründen zu empfehlen:
...
empfangenen Daten ohne eine eindeutige Benutzerkennung nicht über mehrere
Übertragungen zu einem Benutzer zugeordnet werden. Die Zuordnung ist aber
notwendig, wenn man beispielsweise die Bewegungen von Personen über einen
längeren Zeitraum überwachen möchte. Ohne eine Benutzerkennung wäre dies immer nur
innerhalb eines übertragenden Datensatzes möglich. Um das Problem zu
...
\begin{itemize}
  \item Durch die Benutzerkennung darf man auf keine reale Person schließen
  können.
...
die {\em Action} verwendet werden, die für jedes Plug-In eindeutig ist.

Aus der Kombination der Plug-In spezifischen {\em Action} und der \acs{imsi}
lässt sich nun eine eindeutige Benutzerkennung berechnen. Zur Berechnung eignen sich
...
Konkatenation aus Plug-In-Action und \acs{imsi} und der anschließenden Anwendung
...
Die Kennung soll nun bei jeder Übertragung mit übertragen werden.
