...
entsprechende Schnittstelle auf den Plug-In Service zu, welcher dauerhaft im
...
\begin{itemize}
  ...
  werden die bisher gesammelten Daten für eine Übertragung vorbereitet.
...
versendet jener ein {\em Added-Intent} an den Plug-In Service, dass eine neue
Anwendung installiert wurde. Der {\em Intent} wird von der {\em
onStartCommand}-Methode des Plug-In Services empfangen, welcher daraufhin ein
{\em Find-Intent} an die neu installierte Anwendung versendet. Antwortet die
...
ein Log-Eintrag erstellt. Dieser enthält in benutzerverständlicher Sprache,
...
ein \acs{json}-Objekt sich optimal als Zeichenkette abbilden lässt, kann diese
ohne großen Aufwand per \acs{http} versendet werden.
...
Implementierung ersetzen, weil {\em Android System Services} selbst zur
Kommunikation mit der eigentlichen dahinter liegenden Service-Implementierung
den \acs{ipc}-Mechanismus verwenden. Das ist bei den Klassen zur Ansteuerung der
Kamera und dem Mikrofon nicht der Fall. Das Problem liegt darin, dass die
Klassen zur Verwendung der Kamera und des Mikrofons, die anfallenden Daten in
Dateien speichern. Man könnte nun die Klassen wie die {\em Android System
Service} durch eine sichere Implementierung ersetzen, indem man ebenfalls den
...
wurde zugunsten einer einfacheren Implementierung von Plug-Ins verzichtet.
Stattdessen sollte in diesen Fällen die von Android zur Verfügung gestellten
...
Das Problem würde sich am besten durch die Verwendung von \acs{osgi} lösen
