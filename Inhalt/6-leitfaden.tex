...
\label{cha:leitfaden}
In diesem Kapitel wird erklärt, wie man mithilfe der Eclipse \acs{ide},
Plug-Ins für den {\em Mobile Data Collector} entwickeln kann. Dies schließt
neben der Entwicklung der Smartphone-Komponente auch die Serverkomponente mit
ein. Dieses Kapitel ist in drei Teile gegliedert. Im Ersten wird erklärt, wie
man die nötigen Frameworks zur Entwicklung von Plug-Ins, sowie deren
Serverkomponente beziehen kann. Im zweiten Teil wird erklärt, wie man ein
einfaches Plug-In entwickeln kann und im letzten Teil die dazugehörige
Serverkomponente. Das hierbei entwickelte Plug-In trägt den Arbeitstitel ``Hallo Welt''-Plug-In.
Dieses Plug-In speichert bei jeder Ausführung den Text ``Hallo Welt'' in der
Datenstruktur des {\em Mobile Data Collectors}. Dieses Kapitel verzichtet dabei,
aus Einfachheitsgründen, auf die Verwendung von System Services. Beispiele
dazu können aber dem Abschnitt \ref{cha:evaluation} entnommen werden.
...
oder von GitHub über die \acs{url} \url{https://github.com/mlegenhausen/mdcf}
bezogen werden. Es ist dabei möglich, das Framework als Archiv runterzuladen oder per
Git zu klonen. Bei Git handelt es sich um verteiltes Versionskontrollsystem, das
von \url{http://git-scm.com/} heruntergeladen werden kann.
...
develop}-Branch zu wechseln, muss Folgendes auf der Kommandozeile ausgeführt
werden.
...
der Arbeit die beste Unterstützung bei der Entwicklung von Android Anwendungen
geboten hat. Zur Einrichtung der Eclipse \acs{ide} sind folgende Schritte
durchzuführen.

\begin{enumerate}
  \item Installation des Android \acfp{sdk}. Dazu müssen
...
Workspace importiert. Diese Schritte wiederholt man nun für das {\em
...
man zum {\em New Android Project}-Dialog. In diesem Dialog trägt man unter {\em
Project name} ``HelloWorldPlugin'' ein. Wählt unter {\em Build target} die
...
HelloWorldPlugin}-Projekt machen und den Menüpunkt {\em Properties} auswählen.
In dem sich öffnen Dialog wählt auf der linken Seite den Punkt {\em Android}
...
hinzugefügt werden. Dazu erstellt man unterhalb des {\em HelloWorldPlugin}-Projekts ein
...
geht man wieder in die {\em Properties} des {\em HelloWorldPlugin}-Projekts. Auf
der linken Seite wählt man {\em Java Build Path} aus. Daraufhin wählt man den
Tab {\em Libraries} aus und klickt dort auf {\em Add Jars\ldots}. In dem sich öffnen
Dialog navigiert man zu der zuvor kopierten Bibliothek und wählt diese aus.
Durch einen Klick auf {\em OK} wird die Bibliothek zum {\em Java Classpath}
hinzugefügt und das Projekt ist fehlerfrei.

\subsection{Implementierung des Plug-Ins}
\label{pluginimplementation}
Nun kann das eigentliche Plug-In implementiert werden. Hierzu erstellt man im
...
HelloWorldPlugin} ein und bei {\em Superclass} wählt man mit einem Klick auf
...
gespeichert werden. Dazu fügt man zur {\em onRun}-Methode den Code von Listing
...
Namen ``Value'' hinzugefügt. In der letzten Zeile werden alle Änderungen am {\em Workspace}
wieder gespeichert.
...
HelloWorldRegister}, die von {\em XMLPluginRegister} erbt. Zu dieser Klasse fügt
man einen {\em Constructor} hinzu, der auf die Datei ``plugin.xml'' verweist.
Diese Datei enthält die Plug-In Informationen im \acs{xml}-Format. Die
...
\begin{itemize}
  \item {\bf action}: Das Plug-In kann innerhalb des Android Systems über die
  {\em action}\\ {\em com.example.helloworld.HELLO\_WORLD\_PLUGIN} per \acs{ipc} angesprochen
  werden.
  \item {\bf version}: Das Plug-In trägt die Version 1.0.
  \item {\bf name}: Der Darstellungsname innerhalb des {\em Mobile Data
  Collectors} ist ``Hello World''.
  \item {\bf period}: Das Plug-In wird alle 10.000ms bzw. alle 10s ausgeführt.
  \item {\bf duration}: Die maximale Ausführungsdauer beträgt 1.000ms. Wird
  diese Zeit über\-schritten wird das Plug-In vom Android System terminiert.
  \item {\bf url}: Alle gesammelten Daten sollen bei einer Übertragung an die
  URL\\ {\em http://HOSTNAME:PORT/receiver} gesendet werden.
  \item {\bf transferInterval}: Nach einer Zeit von 600.000ms bzw. 10min sollen
  alle Daten Übertragen werden. Dies schließt nur die Zeit ein in der das Plug-In aktiv war.
\end{itemize}

Diese Parameter können je nach Anforderung und Konfiguration beliebig abgeändert
werden.

Weitere mögliche Konfigurationsparameter innderhalb des {\em
plugininfo}-Tags sind:

\begin{itemize}
  \item {\bf resetWorkspaceAfterTransfer}: Gibt an, ob der {\em Workspace}
  automatisch gelöscht werden soll, nachdem das Plug-In im {\em Transfer}-Mode
  war. Der {\em Workspace} wird standardmäßig nach der Erstellung einer
  Übertragung gelöscht.
  \item {\bf description}: Eine längere Plug-In Beschreibung.
  \item {\bf services}: Gibt an, welche sicheren Android Services bei der
  Plug-In Aus\-führung gesetzt werden sollen.
\end{itemize}

Die zurzeit verfügbaren sicheren Android System Services sind:
...
\begin{itemize}
  ...
  Informationen über alle Mobilfunkaspekte abzufragen.
\end{itemize}

Ein Beispiel für die Verwendung der Parameter ist in Lisiting
\ref{additionalparameters} dargestellt. In diesem werden alle verfügbaren
Services angefordert und der {\em Workspace} wird nach der Erstellung einer
Übertragung nicht mehr gelöscht.

\begin{lstlisting}[label=additionalparameters, caption=Zusätzliche
...
Ein konkretes Beispiel, indem diese Konfigurationparameter verwendet werden,
...
Einträge erweitert. Einmal um einen {\em Broadcast Receiver} (Zeile 8 bis 14)
...
Data Collector}, die Plug-In-Schnittstelle aufrufen und das Plug-In ausführen.
...
\subsection{Plug-In installieren und ausführen}
...
Durch einen Klick im Kontextmenü auf {\em Run As} und {\em Android Application}
...
Mobile Data Collectors} angezeigt. Aktiviert man nun das Plug-In, wird dieses
das erste mal ausgeführt. Man kann sich nun in den Plug-In Details die gespeicherten
Daten anschauen. Navigiert man in der Datenstruktur herum, wird man
feststellen, dass bei jeder Ausführung die Zeichenkette ``Hello World''
gespeichert wurde.
...
Da die Kommunikation mit dem {\em Mobile Data Collector} über das
\acs{http}-Protokoll stattfindet, wird die Serverkomponente als Web-Anwendung
entwickelt. Hierzu werden normale Java Servlets verwendet, sowie das
...
Serverkomponente. Diese kann mit {\em Maven} wie folgt gebaut werden.
...
Alle auftretenden Fragen können durch die Eingabe von {\em Enter} bestätigt
werden. Leider wird das Projekt unvollständig von {\em Maven} generiert.
Deswegen wird für die weiteren Schritte empfohlen nachträglich ein {\em
...
die in Listing \ref{pomxml} angegebenen Abhängigkeiten zur {\em pom.xml}
hinzugefügt werden.
...
Damit der zuvor implementierte {\em TransferRequestProcessor} über eine
...
Daten über das URL-Suffix {\em /receiver} empfangen werden können. Bei dem {\em
TransferRequestReceiver} handelt es sich um ein Servlet, dass die vom Smartphone
empfangenen Daten, auf das {\em TransferRequest}-Objekt überträgt und an den
definierten {\em TransferRequestProcessor} übergibt. Hierbei ist anzumerken,
dass der Suffix mit der in Listing \ref{helloworldpluginxml} definierten {\em
url} übereinstimmen muss. Letztendlich muss in der ``plugin.xml'' die URL
...
WAR}-Archive erstellt. Dieses lässt sich in einem beliebigen Servlet-Container
ausführen.

Führt man die Serverkomponente aus und übertragt Daten vom {\em Mobile Data
Collector} zum Server, wird man feststellen, dass auf der Console des Servers
