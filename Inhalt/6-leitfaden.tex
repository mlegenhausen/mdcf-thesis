...
Plug-Ins für den {\em Mobile Data Collector} entwickeln kann. Das schließt
neben der Entwicklung der Smartphone-Komponente auch die Serverkomponente mit
ein. Das Kapitel ist in drei Teile gegliedert. Im Ersten wird erklärt, wie
...
Das Plug-In speichert bei jeder Ausführung den Text ``Hallo Welt'' in der
Datenstruktur des {\em Mobile Data Collectors}. Dieses Kapitel verzichtet,
aus Einfachheitsgründen, auf die Verwendung von System Services. Beispiele
dazu können dem Abschnitt \ref{cha:evaluation} entnommen werden.
...
beschrieben. Das schließt das Beziehen der nötigen Software sowie deren
Einrichtung ein.
...
Die Datei enthält die Plug-In Informationen im \acs{xml}-Format. Die
...
\begin{itemize}
  ...
  die Zeit über\-schritten wird das Plug-In vom Android System terminiert.
  ...
  alle Daten Übertragen werden. Das schließt nur die Zeit ein in der das Plug-In
  aktiv war.
\end{itemize}

Die Parameter können je nach Anforderung und Konfiguration beliebig abgeändert
werden. Weitere mögliche Konfigurationsparameter innderhalb des {\em
...
dieses, dass erste mal ausgeführt. Man kann sich nun in den Plug-In Details die
gespeicherten Daten anschauen. Navigiert man sich durch die Datenstruktur, so
wird man feststellen, dass bei jeder Ausführung die Zeichenkette ``Hello World''
gespeichert wurde.
...
Daten über das \acs{url}-Suffix {\em /receiver} empfangen werden können. Bei dem
...
url} übereinstimmen muss. Letztendlich muss in der ``plugin.xml'' die \acs{url}
...
Die Serverkomponente ist nun komplett. Durch ein {\em mvn install} kann das
...
ausführen. Führt man die Serverkomponente aus und übertragt Daten vom {\em
