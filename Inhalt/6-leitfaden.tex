...
\label{cha:leitfaden}
In diesem Kapitel wird erklärt, wie man mit Hilfe der Eclipse \acs{ide},
Plug-Ins für den {\em Mobile Data Collector} entwickeln kann. Dies schließt neben der
Entwicklung der Smartphone Komponente auch die Serverkomponente mit ein. Dieses
Kapitel ist in drei Teile gegliedert. Im ersten wird erklärt, wie man die
nötigen Frameworks zur Entwicklung von Plug-Ins, sowie deren Serverkomponente
beziehen kann. Im zweiten wird erklärt wie man ein einfaches ``Hallo
Welt''-Plugin entwickeln kann und im letzten die dazugehörige Serverkomponente.
...
\begin{enumerate}
  \item Installation des Android \acfs{sdk}. Dazu müssen
...
HelloWorldPlugin}-Projekt machen und {\em Properties} auswählen. In dem sich
öffnen Dialog wählt auf der linken Seite den Punkt {\em Android} aus. Dort kann man
...
Klasse wie in Listening \ref{helloworldplugin} erstellt.
...
gespeichert werden. Dazu fügt man zur {\em onRun}-Methode den Code von Listening
...
hinzugefügt. In der letzten Zeile werden alle Änderungen am {\em Workspace}
wieder gespeichert.
...
Diese Datei enthält die Plug-In Informationen beschrieben in \acs{xml}. Die
Klasse sollte nun wie in Listening \ref{helloworldregister} aussehen.
...
werden. Hierzu erstellt man eine \acs{xml}-Datei im zuvor erwähnten Package. Es
ist darauf zu achten, das sich die \acs{xml}-Datei innerhalb des {\em Java
Classpath} befindet. In die ``plugin.xml'' ist das Listening \ref{helloworldpluginxml} zu
kopieren.
...
erste mal ausgeführt. Man kann sich nun in den Plug-In Details die gespeicherten
...
dass der Suffix mit der in Listening \ref{helloworldpluginxml} definierten {\em
url} übereinstimmen muss. Letzendlich muss in der ``plugin.xml'' des Plug-Ins
