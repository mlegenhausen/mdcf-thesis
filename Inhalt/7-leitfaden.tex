...
\begin{enumerate}
  \item Installation des {\em Android \acfp{sdk}}. Dazu müssen
  die Schritte auf \url{http://developer.android.com/sdk/installing.html}
  befolgt werden.
  \item Installation des {\em \acf{adt}} Plug-Ins für Eclipse.
...
In dem sich öffnenden Dialog wählt man auf der linken Seite den Punkt {\em
...
öffnenden Dialog navigiert man zu der zuvor kopierten Bibliothek und wählt diese
...
\begin{itemize}
  ...
  alle Daten Übertragen werden. Das schließt nur die Zeit ein, in der das
  Plug-In aktiv war.
\end{itemize}
...
Anschließend muss das Plug-In und der {\em Broadcast Receiver} dem
...
Einträge erweitert. Um einen {\em Broadcast Receiver} (Zeile 8 bis 14), zum
...
Im nächsten Schritt kann das eigentliche Plug-In installiert werden. Dazu
...
dieses, das erste Mal ausgeführt. Man kann sich nun in den {\em Plug-In
...
anderen, dass die Daten über das \acs{url}-Suffix {\em /receiver} empfangen
werden können. Bei dem {\em TransferRequestReceiver} handelt es sich um ein
Servlet, das die vom Smartphone empfangenen Daten auf das {\em
TransferRequest}-Objekt überträgt und an den definierten {\em
TransferRequestProcessor} übergibt. Hierbei ist anzumerken, dass der Suffix mit
der in Listing \ref{helloworldpluginxml} definierten {\em url} übereinstimmen
...
WAR}-Archiv erstellt. Dieses lässt sich in einem beliebigen Servlet-Container
