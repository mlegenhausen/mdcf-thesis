...
optimmierte Virtuelle Maschine.
...
\begin{itemize}
  ...
  der Bildschirm aus bzw. ein geschaltet wurde. Anwendungen selbst können ebenfalls
  Broadcasts verschicken, durch die andere Anwendungen über ein Ereignis
...
den Browser mit einer bestimmten URL zu starten.
...
innerhalb der Anwendung nicht verwendtbar. Innerhalb des Manifests werden
...
Dies kann direkt geschehen, durch die Angabe der Komponenten-Klasse oder
...
beschriebene Aufgabe selbst zu finden und zu starten. Sollten mehre
Komponenten die Aufgabe erfüllen können, so wird der Benutzer gefragt welche Komponente
verwendet werden soll. Dieser Mechanismus kann innerhalb des Manifests durch
{\em intent filter} definiert werden. Dabei werden die entsprechenden {\em
...
\begin{itemize}
  ...
  \item Jede Anwendung läuft in ihrer eigenen \acs{vm}, so das jede
...
auf welche gemeinsamgenutzen Ressourcen diese zugreifen möchten. Diese Rechte
werden in dem jeweiligen Manifest der Anwendung definiert und werden mit der
nach der Installation generierten User ID der Anwendung verknüpft. Die erteilten
...
funktioniert sowohl bei der Kommunikation mit Komponenten der selben Anwendung,
...
Serialiserung alle Parameter und Rückgabewerte~\cite{androidaidl}.
...
personenbezogene Daten und systemkritische Konfigurationen zugegriffen werden kann, sind diese Services durch
...
und per \acs{gps} unterschieden, da die Bestimmung per Mobilfunk um einiges
...
\acs{osgi} auf Android. Die Ausführungsumgebung der Bundles wird auch als OGSi
Container bezeichnet. Folgende \acs{osgi} Container sind unter Android
lauffähig.
...
durch ihre geringe Größe von nur 600KB für die Einbettung in Anwendungen eignet. Durch
ihre geringe Größe eignet sie sich auch für die Verwendung auf mobilen Geräten
