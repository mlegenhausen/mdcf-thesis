...
der Dalvik VM. Die Dalvik VM ist eine für den mobilen Einsatz optimmierte
Virtuelle Maschine.

Die Android Klassenbibliothek ist der, des Java Development Kits nachempfunden
und enthält alle wichtigen Standardkomponenten, sowie Erweiterungen für z.B. die
...
\begin{itemize}
  \item Eine {\bf Activity} repräsentiert eine Bildschirm mit
  ...
  an einem beliebigen anderen Ort gespeichert werden können.
  ...
  Bildschirm aus bzw. ein geschaltet wurde. Anwendungen selbst können ebenfalls
  Broadcast verschicken, durch die andere Anwendungen über ein Ereignis
  imformiert werden können. {\em Broadcast Receiver} besitzen keine
...
Komponente zur eigenen Anwendung gehört oder nicht. Ein Intent ist ein einfaches
Objekt, dass eine Nachricht an eine oder mehre Komponenten spezifiert. Dabei
kann ein {\em Intent} beliebige Daten enthalten, die zur Erfüllung der Aufgabe
in der anderen Komponente benöigt werden. Dies kann z.B. dazu genutzt werden,
...
durch sogenannte {\em Actions}. Durch {\em Actions} ist es möglich einen
Aufgabentyp zu beschreiben der es dem System erlaubt, die Komponente für die
beschriebene Aufgabe zu finden und diese zu starten. Sollten mehre Komponenten
die Aufgabe erfüllen können, so wird der Benutzer gefragt welche Komponente
...
Actions} für jede Komponente definiert, auf die sie reagieren soll.
...
(IPC) eingegangen, da diese zum Verständnis der Masterarbeit erforderlich sind.

\subsection{Das Security-Konzept von Android}
\label{androidsecurity}
Jede installierte Anwendung läuft auf Android in einer Sanbox. Diese Sandbox
...
\begin{itemize}
  ...
  \item Das bedeutet das jede Applikation eine eindeutige Linux User ID erhält.
  Das System setzt dabei Zugriffsrechte für jede Datei der Anwendung, so dass
  nur die Anwendung selbst diese Dateien zugreifen darf.
  \item Jede Anwendung läuft in ihrer eigenen Virtuellen Maschine, so das
  Anwendungens isoliert von anderen Anwendungen läuft.
\end{itemize}

Diese Sandbox erlaubt es sogar nativen Code einer Anwendung auszuführen, ohne
größere Schäden am Gesamtsystems zu befürchten. Durch diese Eigenschaften
...
muss, die nötigt sind, um die eigentliche Aufgabe zu lösen und nicht mehr.

Die gemeinsame Nutzung von Resourcen, wie z.B. die Kamera, GPS oder der
Internetzugriff, werden über ein zusätzliches Rechtesystem geregelt. Bei diesem
müssen Anwendungen bei der installation dem System und dem Benutzer mitteilen
auf welche gemeinsamgenutzen Resourcen diese zugreifen möchte. Diese Rechte
werden in dem jeweiligen Manifest der Anwendung definiert und werden mit der
nach der Installation generierten User ID der Anwendung verknüpft. Diese lassen
sich nach der Installation nicht mehr ändern~\cite{isecsecuringandroidapps}.
...
mit Komponenten anderer Anwendungen, also über Prozessgrenzen hinaus. 
...
Zweck gibt es die Inter Process Communication (IPC). Bei dieser wird über eine Interface
Definition Language (IDL), der Android Interface Definition Language (AIDL), die
Schnittstelle definiert, über die die Kommunikation stattfinden soll. Bei der
Verwendung von AIDL kümmert sich Android um die Serialiserung alle Parameter und
Rückgabewerte~\cite{androidaidl}.
...
Schnittstelle zur Verfügung. Der Zugriff auf die Resource kann dabei
konkurrierend stattfinden. Über die Services kann zum einen auf die Sensorik
zugegriffen werden, aber auch auf systeminterne Dienste. Da über die Sensorik
...
Zugriffsrechte geschützt. Die überprüfung der Zugriffsrechte findet dabei
...
Die von der OSGi Alliance (Früher Open Services Gateway initiative) spezifizierte
Service Plattform ist ein dynamisches Modulsystem für Java, das die dynamische Integration
und das Fernmanagement von Softwarekomponenten (Bundles) und Diensten
(Services) ermöglicht~\cite[Seite 21]{osgiwuetherich}.
...
\begin{itemize}
  ...
  sind. Diese Schicht gehört nicht zur Kernkomponenten der
  OSGi-Plattform und ist somit optional.
  \item \textbf{\em Execution Environment:} Diese Schicht spezifiziert die
  Mindestanforderungen an die Laufzeitumgebung, zur Ausführung der OSGi
  Service Plattform.
\end{itemize}

\subsection{Verfügbare OSGi Container für Android}
Dadurch das Android auf einer Teilmenge des Apache Harmony Projektes basiert,
deckt es einen Großteil der Standard Java
Klassenbibliothek~\cite{apacheharmony}. Dies erlaubt die Ausführung von OSGi auf
Android. Die Ausführungsumgebung der Bundles wird auch als OGSi Container
bezeichnet. Folgende OSGi Container sind unter Android lauffähig.
...
werden dabei in Form von Bundles installiert. Das Framework kümmert dabei um den
Lebenszyklus eines Plug-Ins und erlaubt das senden von Kontextinformationen an
registrierte Anwendungen. Der vom Dynamix Framework zur Verfügung gestellt OSGi
Container bietet eine integration in das Android System. Er läuft dabei als
...
db4o ist eine Objektdatenbank für die Java und .NET Plattform, die sich durch
ihre geringe Größe von nur 600KB für die Einbettung in Anwendungen eignet. Durch
ihre geringe Größe eignet sie sich auch für die Verwendung von mobilen Geräten
...
ähnlichen Objekten gesucht. Bei Critera Queries werden SQL Queries anhand von
entsprechenden Methodenaufrufen nachgebildet und bei Nativen Queries können
