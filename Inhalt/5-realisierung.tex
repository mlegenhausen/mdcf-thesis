...
Ein Plug-In wird dadurch definiert, dass es das {\em Plugin}-Schnittstelle
...
einer {\em run}-Methode, mit der das Plug-In ausgeführt wird. Da die {\em
Setter}-Implementierungen trivial und in den meisten Fällen identisch sind, gibt
es zur Vereinfachung die Klasse {\em AbstractPlugin}. Erbt man von dieser Klasse muss
...
{\em Braodcast Receiver} implementiert werden. Zu diesem Zweck existiert bereits
...
zurück liefern, welche die Plug-In Informationen enthält. Bei dem {\em
PluginInfo}-Objekt handelt es sich um ein Java \acs{pojo}, dass die in Abschnitt
\ref{pluginsystemforandroid} beschriebenen Attribute beistzt. Neben der reinen
...
gesammelten Daten in einer \acs{db4o}-Datenbank (gelb). Sollte ein
Plug-In auf die Android System Services (grau) zugreifen, geschieht dies, aus transparenz
...
Fülle von {\em Activities} (grün) zur Verfügung. Diese {\em Activties} sind:
...
Schnittstelle auf diesen Service zu. Der {\em Plug-In Service} läuft
...
\begin{itemize}
  ...
  Betriebssystem benötigt. Dieses Attribut sollte im besten Fall leer sein, da
  dies bedeutet, dass das Plug-In nicht auf reglementierte Ressourcen zugreifen
  muss.
  \item {\bf Log Records} speichert alle Zugriffe, die auf die sicheren
...
Record} und alle gesammelten Daten enthält, wird diese Datenstruktur 
...
Der Vorgang der Plug-In Registierung ist in Abbildung \ref{register} dargestellt.
...
versendet dieser ein {\em Added-Intent} an den Plug-In Service, das eine neue
...
daraufhin ein {\em Find-Intent} an die neuinstallierte Anwendung. Antwortet die
Anwendung nicht, so handelt es sich nicht um ein Plug-In. Antwortet die
neuinstallierte Anwendung mit einem {\em PluginInfo}-Objekt, so handelt es sich
...
gespeichert und das Plug-In gilt somit als registriert. Die weitere Verwaltung
...
kann. Als Thread-Pool Implementierung, wurde der {\em
...
zuvor erwähnten Thread-Pool zur sofortigen Ausführung übergeben. Bei der
Ausführung wird aus der {\em PluginInfo} die {\em Action} ausgelesen. Mit dieser
ist es Möglich die Service Schnittstelle des Plug-Ins aufzurufen. Konnte eine
Verbindung zum Plug-In hergestellt werden, werden zuerst alle für die Ausführung
nötigen Objekte gesetzt, um daraufhin das Plug-In auszuführen. Das Plug-In wird
dabei nur maximal für die im {\em PluginInfo} angegebene Zeit ausgeführt. Sollte
diese Zeit überschritten werden, so wird das Plug-In vom Android System
terminiert. Nach der Ausführung wird das Plug-In wieder zum Thread-Pool
hinzugefügt, um ihn aber diesmal erst nach dem vom Plug-In definierten Abstand
auszuführen. Dies wiederholt sich solange bis das Plug-In deaktiviert wurde oder
eine Übertragung vorbereitet wird. Dadurch, dass das Plug-In immer im nachhinein
wieder zum Thread-Pool hinzugefügt und nicht automatisch periodisch ausgeführt
wird, wird eine Überlappung der Plug-In Ausführungen verhindert.
...
installert werden. Um den Android Market schneller zu erreichen, kann wie in
Abbildung \ref{more_plugins}, über die hardwareseitige {\em Menü}-Taste direkt
auf diesen zugegriffen werden.
...
eine höhere Ebene zu gelangen, kann die hardwareseitige Zurücktaste verwendet werden.
...
Übertragung angezeigt Daten gehen daraufhin unwiederruflich verloren.

Intern wird bei der Übertragung zu einem Server eine HTTP Verbindung aufgebaut.
Das Verbindungsziel wird hierbei dem URL Attribut der Plug-In Informationen
entnommen. Die Daten die in Form von Java Objekten vorliegen, werden dann mit
Hilfe der GSON Bibliothek in ein \acs{JSON}-Objekt übersetzt. Da sich dabei nur
um eine Zeichenkette handelt, kann dieser einfach per HTTP versendet werden.
...
Servlet} empängt die vom {\em Mobile Data Collector} versandte \acs{json}
...
werden. Diese Hardware wie z.B. \acs{gps} oder der Lagesensor werden aus diesen
...
eigentlichen dahinter liegende Service Implementierung den \acs{ipc} Mechanimus
...
indem man ebenfalls den \acs{ipc} Mechanimus verwendet, um die Ausführung in den
{\em Mobile Data Collector} zu verschieben. Hierbei ergibt sich aber nun das Problem,
...
um Plug-Ins den Zugriff auf die Dateien zu ermöglichen.
...
gespeichert werden müssen, entfällt hierbei nur die Möglichkeit des Loggings.
